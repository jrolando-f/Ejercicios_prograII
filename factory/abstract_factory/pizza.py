...
class Pizza(ABC):
    def __init__(self, name: str, ing_factory: PizzaIngredientFactory):
        ...
        self.dough=None; self.sauce=None; self.cheese=None; self.clam=None; self.veggie = None; self.pepperoni = None
    @abstractmethod
    def prepare(self): ...
...
class VeggiePizza(Pizza):
    def prepare(self):
        print(f"Preparing {self.name}")
        self.dough=self.f.create_dough(); self.sauce=self.f.create_sauce(); self.cheese=self.f.create_veggie()
        print(" ->", self.dough, "/", self.sauce, "/", self.veggie)
